...
class ShigellaEffect:
    ...
    def setup(self, builder):
        ...
        self.immunity_duration = pd.Timedelta(days=builder.configuration.shigella_vaccine.immunity_duration)
        ...
        self.onset_delay = pd.Timedelta(days=builder.configuration.shigella_vaccine.onset_delay)
        ...
        builder.value.register_value_modifier('cause.shigellosis.incidence_rate', self.apply_vaccine_protection)
    ...
    def apply_vaccine_protection(self, index, incidence_rate):
        ...
        protected = self.population_view['vaccine_dose_count'] >= self.doses_for_protection.loc[index]
        ...
        in_waning = (protected & (time_in_waning > 0))

        in_full_protection = protected & ~in_delay & ~in_waning

        protection.loc[in_full_protection | in_waning] = self.efficacy
        protection.loc[in_waning] *= np.exp(-self.waning_rate * time_in_waning.loc[in_waning])
